function [ backgroundPrimary, modulationPrimary ] = nistSpatialMelMod_makeModulationSPD( calibrationFileName, varargin )
...
%       targeted receptor classes. Set entries to Inf to maximize contrast.
%        
...
p.addParameter('desiredContrast',[Inf],@isnumeric);
...
if p.Results.correctBleaching
    ...
    for p = 1:length(p.Results.photoreceptorClasses)
        switch p.Results.photoreceptorClasses{p}
            case 'LCone'
...
T_receptors = GetHumanPhotoreceptorSS(S, p.Results.photoreceptorClasses, p.Results.fieldSizeDegrees, p.Results.observerAgeInYears, p.Results.pupilDiameterMm, [], p.Results.fractionBleached, [], []);
